...
export function createPerformanceMonitor(): PerformanceMonitor {
	...
	return Object.freeze({
		...
		endTimer(timer: PerformanceTimer): PerformanceMetrics {
			const timerMetrics = timer.end();
			metrics.push(timerMetrics);
			return timerMetrics;
		},
	...
	return new Promise((resolve, reject) => {
		...
		operation()
			.then((result) => {
				...
				const check = monitor.checkThresholds(performanceMetrics);

				resolve({ result, metrics: performanceMetrics, check });
			})
			.catch((error) => {
				reject(error);
			});
